...
/*	Nhận 5 chữ cái ASCII và chuyển chúng thành một word khi nhấn Enter ('\r')	*/
uint16_t getNumber16Bit(void)
{
	// Nhận 1 số 0 - 65536 từ cổng Serial và chuyển chuỗi ký tự thành số tương ứng
	char tenThousands = '0';
	char thousands = '0';
	char hundreds = '0';
	char tens = '0';
	...
	return (uint16_t)(10000 * (tenThousands - '0') + 1000 * (thousands - '0') + 100 * (hundreds - '0') + 10 * (tens - '0') + ones - '0');
}

/*	In ra chuỗi ký tự được lưu trong bộ nhớ FLASH	*/
void printString_PROGMEM(const char* str)
{
	uint8_t data;
